...
def read_checkpoint() -> Optional[datetime]:
    try:
        ...
        cp = json.loads(raw)
        ts = cp.get("last_ingested_timestamp")
        if not ts:
            return None
...
def write_checkpoint(last_ts: str, rows: int):
    try:
        data = {
        ...
        print(f"  Checkpoint written: {json_str}")
...
data_frame = (
    raw_data_frame.select(func.col("value").cast("string").alias("log_fragment"))
...
if last_ts is not None:
    checkpoint_str = last_ts.strftime("%Y-%m-%d %H:%M:%S.%f")
    data_frame = data_frame.filter(func.col("timestamp") > checkpoint_str)
...
def process_batch(batch_df, batch_id):
    ...
    t_write = time.time()
    try:
        batch_df.write.mode("append").partitionBy("year", "month", "day").option(
            "header", "true"
        ).csv(HDFS_BASE)
    ...
    t_cp = time.time()
    max_ts_row = batch_df.agg(func.max("timestamp").alias("max_ts")).collect()[0]
    max_ts = str(max_ts_row["max_ts"])
    write_checkpoint(max_ts, row_count)
    cp_elapsed = time.time() - t_cp
    print(f"  Checkpoint time : {cp_elapsed:.4f}s")
    print(f"  Checkpoint → last_ingested_timestamp = {max_ts}")
...
print(f"    It will read from timestamp > {last_ts}")
print("\nDone!")
